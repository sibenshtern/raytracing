...
| Библиотека  | Версия    | Назначение в проекте                                                                                                                                                                                                                                 | Откуда берётся                                    |
| ----------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| **OpenMPI** | 5.0.9     | Распараллеливание рендера по MPI-рангам: разбиение картинки на полосы строк, сборка результата через `MPI_Gatherv` на ранг 0. Используются `MPI_Init`, `MPI_Comm_rank/size`, `MPI_Barrier`, `MPI_Wtime`, `MPI_Gatherv`, `MPI_Abort`, `MPI_Finalize`. | Устанавливается пользователем (см. п. 3).         |
| **libpng**  | 1.6.43    | Запись итогового RGB8-кадра в `.png` на ранге 0. Обёрнут в [`src/image_io.c`](src/image_io.c).                                                                                                                                                       | Скачивается из GitHub через CMake `FetchContent`. |
| **zlib**    | 1.3.1     | Транзитивная зависимость `libpng` (DEFLATE-компрессия PNG-чанков).                                                                                                                                                                                   | Скачивается из GitHub через CMake `FetchContent`. |

`zlib` и `libpng` собираются как **статические** библиотеки (`zlibstatic`, `png_static`) — итоговый `raytracer` не требует никаких разделяемых `.so`/`.dylib`, кроме OpenMPI и libc.
